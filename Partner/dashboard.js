...
router.post("/", async (req, res) => {
    ...
    try {
        // 1. Fetch all non-cancelled bookings for this agent
        const [bookings] = await db.execute(
            `SELECT * FROM bookings WHERE agent_id = ? AND status != 'Cancelled'`,
            [agent_id]
        );
        ...
        const serviceList = Object.entries(serviceMap).map(([name, count]) => ({
            label: name,
            value: count,
            emoji: "💇‍♀️", // Generic emoji since we don't store emoji in db yet
